...
FrameBuffer* PostEffectBuffers::RequestFramebuffer(uint32_t nameKey, bool addReference)
{
...
void PostEffectBuffers::OnFrameRendered() 
{
	for (auto it = begin(framebufferPool); it != end(framebufferPool); )
	{
		...
		// NOTE: erase counter is reset on AddReference call
		// so we are safe to think that there is a framebuffer not used for N-frames
		if (entry.ReadyToErase())
		{
