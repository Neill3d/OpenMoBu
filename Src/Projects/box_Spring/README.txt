...
EvalSteps - number of spring evaluation substeps per second
 Default value - 30.0

RealTime - checkbox to specify spring evaluation time. When checkbox is on, spring uses system time and play in realtime. When it's off, spring uses player control local time from time slider (this is for plotting and animation playback).
 Default value - on (1.0)

Zero Frame - frame on which we reset our simulation (for non realtime evaluation). This is when we rewind our animation to the very begining and want to start simulation from that "zero" point
 Default value - 0 (frame 0)
...
Result - output position
 ...
 29.05.2014 - compile for almost all versions and fix math for real-time and playback evaluation