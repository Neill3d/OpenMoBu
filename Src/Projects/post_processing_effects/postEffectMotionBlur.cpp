...
constexpr const char* SHADER_MOTIONBLUR_NAME{ "MotionBlur" };
constexpr const char* SHADER_MOTIONBLUR_VERTEX{ "\\GLSL\\simple.vsh" };
constexpr const char* SHADER_MOTIONBLUR_FRAGMENT{ "\\GLSL\\motionblur.fsh" };
...
bool PostEffectMotionBlur::CollectUIValues(UICallback* uiCallback, const SViewInfo& viewInfo)
{
	bool lSuccess = false;
...
//	float RadiusToScreen = R * 0.5f * projScale;
	...
	if (nullptr != mShader)
	{
        ...
        const int localFrame = viewInfo.localTimeFrame;
		
		if (0 == localFrame || (localFrame != m_LastTimeFrame))
		{
			...
			if (mLoc.dt >= 0)
			{
				float dt = static_cast<float>(viewInfo.localTime - m_LastTime);
				dt *= 0.01f * (float)_amount;
				glUniform1f(mLoc.dt, dt);
			}

            m_LastTime = viewInfo.localTime;
			m_LastTimeFrame = viewInfo.localTimeFrame;
		}
