...
constexpr const char* SHADER_SSAO_NAME{ "SSAO" };
constexpr const char* SHADER_SSAO_VERTEX{ "\\GLSL\\simple.vsh" };
constexpr const char* SHADER_SSAO_FRAGMENT{ "\\GLSL\\ssao.fsh" };

constexpr const int AO_RANDOMTEX_SIZE{ 4 };
...
bool PostEffectSSAO::CollectUIValues(UICallback* uiCallback, const SViewInfo& viewInfo)
{
	bool lSuccess = false;
    ...
    float znear = static_cast<float>(viewInfo.nearPlane);
    float zfar = static_cast<float>(viewInfo.farPlane);
	...
	float fov = 2.0f * atan(diag / (focallen * 2.0f));
	...
	clipInfo[2] = zfar;
	clipInfo[3] = (viewInfo.isPerspective) ? 1.0f : 0.0f;
	...
	int useOrtho = (false == viewInfo.isPerspective) ? 1 : 0;
...
bool PostEffectSSAO::InitMisc()
{
	...
	float Rand1 = static_cast<float>(dist(e2));
	float Rand2 = static_cast<float>(dist(e2));
	...
	for (int i = 0; i < HBAO_RANDOM_SIZE; i++)
	{
		...
		{
			Rand1 = static_cast<float>(dist(e2));
			Rand2 = static_cast<float>(dist(e2));
