
#pragma once

// postprocessing_effectLensFlare
/*
Sergei <Neill3d> Solokhin 2018-2022

GitHub page - https://github.com/Neill3d/OpenMoBu
Licensed under The "New" BSD License - https://github.com/Neill3d/OpenMoBu/blob/master/LICENSE
*/

#include "postEffectBase.h"

///////////////////////////////////////////////////////////////////////////////////////
// PostEffectLensFlare

class PostEffectLensFlare : public PostEffectBase
{
public:
	static const int NUMBER_OF_SHADERS{ 3 };

	//! a constructor
	PostEffectLensFlare();

	//! a destructor
	virtual ~PostEffectLensFlare();

	int GetNumberOfShaders() const override { return NUMBER_OF_SHADERS; }

	const char *GetName() override;
	const char *GetVertexFname(const int shaderIndex) override;
	const char *GetFragmentFname(const int shaderIndex) override;

	virtual bool PrepUniforms(const int shaderIndex) override;
	virtual bool CollectUIValues(UICallback* uiCallback, const SViewInfo& viewInfo) override;

	virtual const int GetNumberOfPasses() const override;
	virtual bool PrepPass(const int pass) override;

protected:

	//Louis
	EFlareType		FlareType;
	//FBSystem		mSystem;

	// shader locations

	struct SubShader
	{
	public:
		enum { LOCATIONS_COUNT = 13 };
		union
		{
			struct
			{
				// locations
				//Louis
				GLint		seed;

				GLint		upperClip;
				GLint		lowerClip;

				GLint		amount;

				GLint		textureWidth;
				GLint		textureHeight;

				GLint		timer;

				GLint		light_pos;

				GLint		tint;
				GLint		inner;
				GLint		outer;

				GLint		fadeToBorders;
				GLint		borderWidth;
				GLint		feather;
			};

			GLint		mLocations[LOCATIONS_COUNT];
		};

		FBSystem mSystem;


		int				m_NumberOfPasses;
		float			m_DepthAttenuation;

		std::vector<FBVector3d>	m_LightPositions; // window xy and depth (for attenuation)
		std::vector<FBColor>	m_LightColors;


		void Init();
		bool PrepUniforms(GLSLShader* mShader);
		bool CollectUIValues(const int shaderIndex, GLSLShader* mShader, UICallback* uiCallback, const SViewInfo& viewInfo);
		bool PrepPass(GLSLShader* mShader, const int pass);
	};

	SubShader	subShaders[NUMBER_OF_SHADERS];
};
