...
class MainForm( QtWidgets.QWidget, Ui_ReferencingSample ):
    ...
    def OnBtnBrowsePathClicked( self ):
        lFileToLoad = QtWidgets.QFileDialog.getOpenFileName(self, "Pick FBX to reference", self.mDefaultPath, "*.fbx" )[0]      # Returns a Tuple. Get the filename only
    ...
    def OnBtnInstanceClicked( self ):
        ...
        if lNSObj.GetContentModified( FBPlugModificationFlag.kFBContentAllModifiedMask ):
            lMsgBox = QtWidgets.QMessageBox( QtWidgets.QMessageBox.Question, 'Instancing', 'Do you want to apply reference edits after instancing?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, self )
    ...
    def UpdateTreeNamespace( self ):
        ...
        if self.mSingleSelection:
            ...
            if isinstance(lNSObj, FBFileReference):
                lShadersItem = QtWidgets.QTreeWidgetItem( ['Shaders Graph'] )
